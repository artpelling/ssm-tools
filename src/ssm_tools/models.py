...
class RustStateSpaceModel(StateSpaceModel):
    """State-space model that uses a compiled Rust solver backend.

    The solver performs the discrete-time recursion::

        y[:, i] = C @ x + D @ u[:, i]
        x       = A @ x + B @ u[:, i]

    The system state ``x`` is updated in place after each call to
    :meth:`process`, so sequential calls carry state across chunk boundaries.
    ...
    dtype : numpy.dtype, optional
        Working dtype; must be ``float32`` or ``float64``.
        Inferred from the matrices when not provided.
    storage : {'F', 'C'}, optional
        Memory layout for the system matrices (``'F'`` for column-major,
        ``'C'`` for row-major).  Defaults to ``'F'``.
    ...
    def __init__(self, A, B, C, D=None, sampling_rate=None, state=None, dtype=None, storage="F", comment=""):
        ...
        if dtype not in [np.dtype(t) for t in self._SUPPORTED_DTYPES]:
            raise TypeError(
                f"RustStateSpaceModel only supports float32 and float64, got {dtype}."
            )
        A, B, C, D = (
            A.astype(dtype, order=storage),
            B.astype(dtype, order=storage),
    ...
    @classmethod
    def from_pyfar(cls, sys: StateSpaceModel, storage="F"):
        """Construct a :class:`RustStateSpaceModel` from a pyfar :class:`StateSpaceModel`.
